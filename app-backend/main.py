...
from consultas import (
    ...
    insertar_promocion,
    verificar_replicacion_promociones,
)
...
@aplicacion.route("/api/promociones", methods=["GET", "POST"])
def api_promociones():
    if request.method == "GET":
        try:
            datos, tiempo_ms = consultar_promociones()
            return respuesta_correcta(datos, tiempo_ms)
        except Exception as error:
            return respuesta_error(error)
    ...
    try:
        ...
        campos_requeridos = ["restaurante_id", "titulo"]
        ...
        return respuesta_error(
            "El campo restaurante_id debe ser un número entero.",
            400
        )
...
@aplicacion.route("/api/replicacion/promociones", methods=["GET"])
def api_replicacion_promociones():
    try:
        datos, tiempo_ms = verificar_replicacion_promociones()
        return respuesta_correcta(datos, tiempo_ms)
    except Exception as error:
        return respuesta_error(error)
